...
def detection_callback(data):
    ...
    for detection in data.detections:
        for result in detection.results:
            ...
            rospy.loginfo("Detection: ClassID=%d, Confidence=%.2f, Center: (%.2f, %.2f)", 
                          class_id, confidence, center_x, center_y)
    ...
    # 打印完成后退出程序
    #rospy.signal_shutdown("Detection processed. Exiting.")
...
def listener():
    ...
    rospy.spin()  # 保持节点运行，直到被手动停止或回调函数中调用 rospy.signal_shutdown
